...
if not os.path.exists(OUTPUT_IMGan_DIR):
    os.makedirs(OUTPUT_IMGan_DIR)
...
def extract_ros_pose(pose):
    ...
    quat_xyzw = np.array([quat.x, quat.y, quat.z, quat.w])  # Scalar-first
    tvec = np.array([pose.pose.position.x, pose.pose.position.y, pose.pose.position.z])
    return quat_xyzw, tvec


def project_keypoints(image, pose):
    rvec = quaternion_to_rvec(pose.pose.orientation)
    tvec = np.array([pose.pose.position.x, pose.pose.position.y, pose.pose.position.z])
...
def process_image():
    ...
    try:
        ...
        original_filename = os.path.join(OUTPUT_IMG_DIR, "image_{0}.jpg".format(timestamp))
        ...
        if ids is not None:
            ...
            for i, marker_id in enumerate(ids):
                ...
                cv2.putText(annotated_image, "ID:{0}".format(marker_id[0]), tuple(center),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255, 0, 0), 2)

        # Save annotated image
        annotated_filename = os.path.join(OUTPUT_IMGan_DIR, "imagean_{0}.jpg".format(timestamp))
        cv2.imwrite(annotated_filename, annotated_image)

        # Publish annotated image
        if annotated_image is not None:
            try:
                rospy.logdebug("Debug - Annotated image shape: {0}, type: {1}".format(
        ...
        # Save JSON data
        json_filename = os.path.join(OUTPUT_JSON_DIR, "image_{0}.json".format(timestamp))
        data = {
            "timestamp": timestamp,
            "original_image_path": "image_{0}.jpg".format(timestamp),
            "translation": [
        ...
        rospy.logerr("Processing error: " + str(e))
